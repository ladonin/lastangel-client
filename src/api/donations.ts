...
export const DonationsApi = {
  ...
  update: (id: number, data: TCommonDataRequest) =>
    apiService
      .post(`update_donation?id=${id}`, data)
      .then((response: AxiosResponse<boolean>) => response.data),
  remove: (id: number) =>
    apiService
      .post(`remove_donation?id=${id}`)
      .then((response: AxiosResponse<boolean>) => response.data),
