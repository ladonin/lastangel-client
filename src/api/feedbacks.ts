...
import { TGetListOutput, TGetListRequest, TCommonDataRequest, TGetNewCountOutput } from "api/types/feedbacks";
import { apiService } from "./axios";

export const FeedbacksApi = {
  getList: (params?: TGetListRequest) =>
    apiService.get(`get_feedbacks_list`, { params }).then((response: AxiosResponse<TGetListOutput>) => response.data),
  add: (data: TCommonDataRequest) =>
    apiService.post(`add_feedback`, data).then((response: AxiosResponse<boolean>) => response.data),
  getNewCount: () =>
    apiService.get(`get_feedbacks_new_count`).then((response: AxiosResponse<TGetNewCountOutput>) => response.data),
